...
def attach_content(email_obj, email_content, content_type='plain', charset='utf-8'):
    '''
    创建邮件正文，并将其附加到根容器：邮件正文可以是纯文本，也可以是HTML（为HTML时，需设置content_type值为 'html'）
...
if __name__ == "__main__":
    # （QQ邮箱）
    email_host ='imap.exmail.qq.com'  # smtp 邮件服务器
    host_port = 465  # smtp 邮件服务器端口：SSL 连接
    from_addr = 'zonglin_lin@mrxdtech.com'  # 发件地址
    pwd = 'MRXDlzl123'  # 发件地址的授权码，而非密码
    ...
    to_addr_list = ["249837922@qq.com"]  # 收件地址
    ...
    attach_content(email_obj, email_content_html, content_type='html')
